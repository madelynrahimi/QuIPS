...
#@njit(cache=True)
def sme_evolution_2D(del_t,period_number, eta, k, gamma, x0, p0, feedback, filt=None, M=None):
    wx_real = 50000 * 2*np.pi  #rad/s
    wy_real = 20000 * 2*np.pi  #rad/s
    m_bead = 4.8e-18 #kg
    hbar = 1.054e-34 #Js
    ...
    p = np.array(p0,dtype=float).reshape(2,1)
    alpha_x = x.copy() 
    ...
    if feedback == 'force':
        ...
        for i in range(steps-1):
            ...
            phi_rot = np.arctan2(p_rot, x_rot)
            phi_p_rot = np.pi/2 - 2*phi_rot
            phi_p_rot_swapped = phi_p_rot[[1, 0], :]
    ...
    var_x_real = Sx[None, :, :] @ var_x_arr @ Sx.T[None, :, :]
    var_p_real = Sp[None, :, :] @ var_p_arr @ Sp.T[None, :, :]
    cov_xp_real = Sx[None, :, :] @ cov_xp_arr @ Sp.T[None, :, :]

    exp_x_sq_real = Sx[None, :, :] @ exp_x_sq @ Sx.T[None, :, :]
    exp_p_sq_real = Sp[None, :, :] @ exp_p_sq @ Sp.T[None, :, :]
...
def plot_sme_traj_2D(del_t,period_number, eta, k, gamma, x0, p0, feedback, filt=None, M=None):
    t_real_arr, t_per_arr,exp_x_real,exp_p_real, alpha_x_stream_real, alpha_p_stream_real, var_x_real, var_p_real, cov_xp_real,exp_x_sq_real,exp_p_sq_real = sme_evolution_2D(del_t=del_t, period_number=period_number, eta=eta, k=k, gamma=gamma, x0=x0, p0=p0, feedback=feedback, filt=filt)
    if not filt:
...
def effective_temp_plot(del_t,period_number, eta, k, gamma_list, x0, p0, feedback, filt=None, m_bead=4.8e-18, w_real=[50000*2*np.pi, 20000*2*np.pi], M= None, plot_bool = False):
    Tx_list = []
    Ty_list = []
    for gamma in gamma_list:
        t_real_arr, t_per_arr,exp_x_real,exp_p_real, alpha_x_stream_real, alpha_p_stream_real, var_x_real, var_p_real, cov_xp_real,exp_x_sq_real,exp_p_sq_real = sme_evolution_2D(del_t=del_t, period_number=period_number, eta=eta, k=k, gamma=gamma, x0=x0, p0=p0, feedback=feedback, filt=filt, M=None)
        dimensions = ["x","y"]
        for i,dim in enumerate(dimensions):
